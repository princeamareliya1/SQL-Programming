...
SELECT * FROM STUDENT
WHERE SPI>8.0   AND BRANCH IN('CIVIL','MECHANICAL')
...
--CIVIL branch.
SELECT * FROM STUDENT
WHERE SPI>8.0 AND  BRANCH<>'CIVIL'
...
--104 and 108.
SELECT * FROM STUDENT
WHERE SPI BETWEEN 7.5 AND 9 AND STDID NOT BETWEEN 104 AND 108
...
--7.0.
SELECT * FROM STUDENT
WHERE CITY<>'RAJKOT' AND SPI>7.0 AND BRANCH='COMPUTER'
...
WHERE CITY='RAJKOT'  OR CITY='SURAT' AND  SPI<8.0 
...
SELECT * FROM STUDENT
WHERE spi>8AND	 BRANCH in('COMPUTER','civil')
...
SELECT SNAME FROM STUDENT
WHERE CITY NOT  IN('SURAT','RAJKOT')
